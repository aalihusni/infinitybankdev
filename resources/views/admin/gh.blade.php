                ...
                <table class="table table-striped table-bordered table-hover" id="dataTables-share">
                    ...
                    @foreach($match_list as $ph)
                        <tr class="odd gradeX">
                            ...
                            <td><div id="GH{{ $ph->id }}"></div><button class="phgh" data-id="{{ $ph->id }}">PHGH</button></td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
...
<script>
    ...
    function get_phgh(ph_id){
        var loadUrl = '{{ URL::to('/') }}/admin/@if (strpos(Request::url(),'micro') > 0){{ "micro-getphgh" }}@else{{  "getphgh" }}@endif/gh/'+ph_id;
        $.ajax({url: loadUrl, success: function(result) {
            $('#GH'+ph_id).html(result);
        }});
    }
</script>
@Stop