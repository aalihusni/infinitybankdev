...
class PassportClass
{
    ...
    {
        $waiting_confirmations = BlockioWalletClass::getWaitingConfirmations($user_id, "P");
    ...
    {
        $passport_transactions = Passport::where('user_id', '=', $user_id)
            ->get();
    ...
    public static function getPassportBalance($user_id)
    {
        ...
        if ($user)
        {
            $passport_balance = $user->passport_balance;
    ...
    {
        $passport_price = Settings::select('passport_price')
    ...
    public static function getPaymentDetails($user_id, $passport_quantity)
    {
        ...
        $return['receiving_address']    = BlockioWalletClass::getReceivingAddressUser(1, $user_id, "P", $value_in_btc, $passport_quantity)['receiving_address'];
    ...
    public static function setPassportBalance($user_id, $passport_quantity, $passport_description = "", $secret = "")
    {
        if($passport_quantity <> 0) {
            ...
            if (!empty($secret)) {
                echo $secret."#############<br>";
                $passport = Passport::where('user_id', '=', $user_id)
                    ->where('secret', '=', $secret)
                    ->get();
            ...
            if ($user) {
                ...
                $user->passport_balance = $passport_balance;
                $user->save();
    ...
    public static function addPassportBonus($user_id, $shares_type_id)
    {
        ...
        $value_in_btc = $passport->passport_price;
