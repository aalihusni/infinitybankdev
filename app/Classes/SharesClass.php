...
class SharesClass
{
    ...
    {
        $bonus_total = Shares::select(DB::raw('sum(debit_value_in_btc) as bonus_total'))
    ...
    public static function getSharesTransactions($user_id, $shares_type = "", $shares_type_id = "", $data_type = "")
    {
        $sql = Shares::where('user_id', '=', $user_id);
        if (!empty($shares_type))
        {
            $sql->where(function ($query) use ($shares_type) {
                $query->where('shares_type', '=', $shares_type);
                $query->orwhere('shares_type', '=', $shares_type."-ERROR");
                $query->orwhere('shares_type', '=', $shares_type."-DUPLICATE");
                $query->orwhere('shares_type', '=', $shares_type."-REVERSAL");
            });
                ...
                foreach ($shares as $share)
                {
                    if ($credit_value > $credited_value) {
                        ...
                        */
                        ...
                        self::setShares($user_id, $secret, $shares_type, $shares_type_id, $shares_type_user_id, $shares_type_percent, $value_in_btc, $debit_id);
                    }
                }
                DB::commit();
            }
    ...
    {
        $shares = Shares::select(DB::raw('sum(credit_value_in_btc) as credit_total'))
            ->where('user_id', '=', $user_id)
    ...
    public static function getSharesBalanceTypeAll($user_id)
    {
        ...
        $shares = Shares::select(DB::raw('sum(debit_value_in_btc) as debit_total'), DB::raw('sum(credit_value_in_btc) as credit_total'), 'shares_type')
            ->where('user_id', '=', $user_id)
            ->groupby('shares_type')
            ->get();
    ...
    {
        $shares = Shares::select(DB::raw('sum(debit_value_in_btc) as debit_total'), DB::raw('sum(credit_value_in_btc) as credit_total'))
            ->where('user_id', '=', $user_id)
    ...
    public static function setShares($user_id, $secret, $shares_type, $shares_type_id, $shares_type_user_id, $shares_type_percent, $value_in_btc, $debit_id = "")
    {
        //PR = passport bonus referral
        //PO = passport bonus overiding
        //BTC = deposit from bitcoin
        ...
        if ($value_in_btc <> 0) {
            ...
            $shares_balance = str_replace(' ','',str_replace(',','',$get_shares_balance['shares_balance']));
            ...
            $shares->balance_value_in_btc =  str_replace(' ','',str_replace(',','',number_format(($shares_balance + $value_in_btc), 8)));
            $shares->secret = $secret;
            $shares->save();
        }
    }
}