...
class BankViewController extends Controller
{
    ...
    public function provide_help()
    {
        ...
        $total_ph_pledgeactive = PHGHClass::getPledgeActivePHTotal($user_id);

        return view('member.provide_help')->with('user_details', $user_details)->with('ph_active', $ph_active)->with('ph_ended', $ph_ended)->with('total_ph_active', $total_ph_active)->with('total_ph_pledgeactive', $total_ph_pledgeactive);
    }

    public function provide_help_ajax($ph_id)
    {
        $ph = PHGHClass::getPH($ph_id);

        return view('member.provide_help_ajax')->with('ph', $ph);
    }
    ...
    public function get_help()
    {
        $user_id = Auth::user()->id;
        $user_details = UserClass::getUserDetails($user_id);
        $gh_active = PHGHClass::getGHAll($user_id, "5", "<=");
        $gh_ended = PHGHClass::getGHAll($user_id, "6", ">=");
        $total_gh_active = PHGHClass::getGHTotal($user_id, "5", "<=");

        return view('member.get_help')->with('user_details', $user_details)->with('gh_active', $gh_active)->with('gh_ended', $gh_ended)->with('total_gh_active', $total_gh_active);
    }

    public function get_help_ajax($gh_id)
    {
        $gh = PHGHClass::getGH($gh_id);

        return view('member.get_help_ajax')->with('gh', $gh);
    }
    ...
    public function pairing()
    {
        ...
        return view('member.pairing')->with('pair', $pair)->with('selected', $selected)->with('percent', $percent)->with('bonus_amount', $bonus_amount)->with('bonus_amount_actual', $bonus_amount_actual)->with('active_ph', $active_ph)->with('pair_history', $pair_history);
    }

    public function ajax_phgh($phgh_id)
    {
        $get_payment_details = PHGHClass::getPHPaymentDetails($phgh_id);
